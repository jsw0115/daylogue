...
@Table(
        ...
        uniqueConstraints = {
                @UniqueConstraint(name = "uk_evt_hash", columnNames = {"tok_hash"})
        },
        indexes = {
                @Index(name = "ix_evt_user", columnList = "user_id"),
                @Index(name = "ix_evt_exp", columnList = "exp_utc"),
                @Index(name = "ix_evt_used", columnList = "used_utc")
        }
)
public class EmailVerifyToken extends UtcCreatedEntity {
