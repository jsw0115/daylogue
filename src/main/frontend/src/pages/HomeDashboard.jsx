...
export default function HomeDashboardScreen() {
  ...
  const onLayoutChange = (_, allLayouts) => {
    // editMode가 아니어도 드래그/리사이즈는 막지만, 레이아웃 값은 안전하게 받음
    ...
    <div className="home-dashboard">
      <div className="screen-head">
        <div>
          ...
          <div className="screen-subtitle">진행 상황과 일정/메모를 한눈에 확인하세요.</div>
        </div>

        <div className="screen-actions">
          <button className="btn" onClick={() => navigate("/planner/daily")} type="button">오늘 일간 플래너</button>
          <button className="btn primary" onClick={() => setEditMode((v) => !v)} type="button">
            편집 모드 {editMode ? "ON" : "OFF"}
          </button>
          <button className="btn" onClick={resetLayout} type="button">레이아웃 초기화</button>
        </div>
      </div>
            ...
            <div key={id} className="portlet">
              <div className="portlet__head">
                <div className="portlet__head-left">
                  {editMode && <div className="portlet__drag" title="드래그로 이동">⋮⋮</div>}
                ...
                <div className="portlet__head-right">
                  ...
                  {editMode && (
                    <button className="btn xs danger" onClick={() => setVisible((p) => ({ ...p, [id]: false }))} type="button">
                      숨김
                    </button>
                  )}
                </div>
