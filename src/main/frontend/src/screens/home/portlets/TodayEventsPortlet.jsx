...
export default function TodayEventsPortlet() {
  ...
  const rawTodayCount = useMemo(() => {
    const all = getEvents().filter((e) => !e.deletedAt);
    const tKey = todayKey();
    return all.filter((e) => e.dateKey === tKey).length;
  }, [dataVersion]);
  ...
  const filteredCount = todayEvents.length;

  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
        <Text type="secondary" style={{ fontSize: 12 }}>
          오늘 일정 <b style={{ color: "#111827" }}>{rawTodayCount}</b>개
          {q.trim() || onlyBookmarked || onlyDday || onlyImportant ? (
            <>
              {" "}
              / 표시 <b style={{ color: "#111827" }}>{filteredCount}</b>개
            </>
          ) : null}
        </Text>
      </div>
            ...
            <div key={e.id} className="evtRow">
              ...
              <div className="evtMain">
                ...
                <div className="evtMeta">
                  ...
                  {e.repeatRule && e.repeatRule !== "NONE" ? <Tag icon={<Repeat size={14} />}>반복</Tag> : null}

                  {typeof e.reminderMin === "number" ? (
                    <Tag icon={<Bell size={14} />}>{e.reminderMin}분 전</Tag>
                  ) : null}

                  {e.participantsCount > 1 ? <Tag icon={<Users size={14} />}>{e.participantsCount}명</Tag> : null}

                  {e.isDday ? <Tag icon={<Pin size={14} />}>D-Day</Tag> : null}
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="evtForm">
          ...
          <div className="evtGrid2">
            <div className="evtField">
              ...
              <Input value={form.start} onChange={(e) => setForm((p) => ({ ...p, start: e.target.value }))} placeholder="HH:mm" />
            </div>
            <div className="evtField">
              <div className="evtLabel">종료</div>
              <Input value={form.end} onChange={(e) => setForm((p) => ({ ...p, end: e.target.value }))} placeholder="HH:mm" />
            </div>
          </div>
          ...
          <div className="evtGrid2">
            <div className="evtField">
              ...
              <Input value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))} placeholder="업무/공부/건강..." />
            </div>

            <div className="evtField">
              <div className="evtLabel">공개범위(EVT-003-F03)</div>
              <Select value={form.visibility} onChange={(v) => setForm((p) => ({ ...p, visibility: v }))} style={{ width: "100%" }}>
          ...
          <div className="evtGrid2">
            <div className="evtField">
              ...
              <Select value={form.importance} onChange={(v) => setForm((p) => ({ ...p, importance: v }))} style={{ width: "100%" }}>
            ...
            <div className="evtField">
              ...
              <Select value={form.repeatRule} onChange={(v) => setForm((p) => ({ ...p, repeatRule: v }))} style={{ width: "100%" }}>
          ...
          <div className="evtGrid2">
            <div className="evtField">
              ...
              <Input value={String(form.reminderMin)} onChange={(e) => setForm((p) => ({ ...p, reminderMin: e.target.value }))} placeholder="예: 10 (분)" />
            </div>
            <div className="evtField">
              <div className="evtLabel">참석자 수(EVT-007-F01)</div>
              <Input value={String(form.participantsCount)} onChange={(e) => setForm((p) => ({ ...p, participantsCount: e.target.value }))} placeholder="예: 3" />
            </div>
          </div>

          <div className="evtField" style={{ marginTop: 6 }}>
            <Space>
              <Tag.CheckableTag checked={!!form.isDday} onChange={(v) => setForm((p) => ({ ...p, isDday: v }))}>
